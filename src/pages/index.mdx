...
In the following locations the admin-level accounts have the ability to write or overwrite arbitrary data with little or no restrictions:

`extendTraitArt()` on [L218-L225](https://github.com/Dapp-Wizards/Avastars-Contracts/blob/b2ac9159a034c27b3ccad98fa806af08be37a842/contracts/TraitFactory.sol#L218-L225) and `setAttribution()` on [L152-L162](https://github.com/Dapp-Wizards/Avastars-Contracts/blob/b2ac9159a034c27b3ccad98fa806af08be37a842/contracts/TraitFactory.sol#L218-L225) in TraitFactory. `setTokenUriBase()` on [L130-L138](https://github.com/Dapp-Wizards/Avastars-Contracts/blob/b2ac9159a034c27b3ccad98fa806af08be37a842/contracts/TraitFactory.sol#L218-L225), `setMediaUriBase()` on [L146-L154](https://github.com/Dapp-Wizards/Avastars-Contracts/blob/b2ac9159a034c27b3ccad98fa806af08be37a842/contracts/AvastarMetadata.sol#L146-L154) and `setViewUriBase()` on [L162-L170](https://github.com/Dapp-Wizards/Avastars-Contracts/blob/b2ac9159a034c27b3ccad98fa806af08be37a842/contracts/AvastarMetadata.sol#L162-L170) in AvastarMetadata. `setMetadataContractAddress()` on [L51-L65](https://github.com/Dapp-Wizards/Avastars-Contracts/blob/b2ac9159a034c27b3ccad98fa806af08be37a842/contracts/AvastarTeleporter.sol#L51-L65) in AvastarTeleporter. `setTeleporterContract()` on [L96-L109](https://github.com/Dapp-Wizards/Avastars-Contracts/blob/b2ac9159a034c27b3ccad98fa806af08be37a842/contracts/AvastarPrimeMinter.sol#L96-L109) in AvastarPrimeMinter.

This creates the possibility for token data to be corrupted or omitted, either by accident or by a compromised admin account.

These include:

- In `setTeleporterContract()`, the address can be changed at any time by the `sysAdmin` account. Users may have deposited funds that are sent to a new contract. The `whenPaused()` modifier provides limited assurances as it can be toggled at any time by the admin, even in the same transaction
...
While this does not pose an explicit security risk as long as the admins remain trustworthy, it does undermine the minimized trust model that many NFT users expect. Furthermore, unchecked admin powers present an ongoing threat if their keys are ever compromised. Another risk is the "bus threat" where only two individuals have the ability to update this data. If one or both are unable to act, these components of the contracts will cease to function correctly.

##### Status

The Avastars team has removed the direct ability to overwrite storage. They have also added an `onlyBeforeProd()` modifier on [L41-L44](https://github.com/Dapp-Wizards/Avastars-Contracts/blob/b2ac9159a034c27b3ccad98fa806af08be37a842/contracts/TraitFactory.sol#L41-L44) in TraitFactory that provides users with greater assurances that their token's data will not change after minting in that generation and series has begun. However, there remains the risk of a generation accidentally or maliciously begining before traits are added.This would prevent the correct traits and other metadata from being added.
...
The AvastarPrimeMinter (and presumably AvastarReplicantMinter) contracts must have minter permissions in the AvastarTeleporter contract to successfully forward user purchases. A smart contract with elevated permissions and access to the interface methods of the AvastarTeleporter contract presents a wider attack surface. Hijacking the Minter's execution context to make a permission call as `msg.sender` is one example. Minter contracts can be `paused` or `upgraded` but minter status cannot be revoked.
