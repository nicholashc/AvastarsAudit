...
Avastars attempts to radically extend the metadata associated with NFT projects by taking the bold action of storing svg layers on-chain that can be recombined into the full artwork. The ambition of such a project attempts to solve an ongoing issue in NFT projects where the user owns an arbitrary data string but none of the art or other cultural associations that make up much of their appeal as collectables.
