...
> 05/02/20
>
> Addressed in 3 out of 4 functions. Not addressed in `approveTraitAccess()` where it is still possible to approve handlers for Replicants trait access. As this is the only function that modifies storage, I recommend adding a check for `Avastar.wave == Wave.PRIME`.

##### Location

`approveTraitAccess()` on [L97-L110](https://github.com/Dapp-Wizards/Avastars-Contracts/blob/b8220ee9335167fd3e5d6c5691b20a3a4bb2a3dc/contracts/AvastarTeleporter.sol#L97-L110) AvastarTeleporter, `getPrimeByTokenId()` on [L79-L102](https://github.com/Dapp-Wizards/Avastars-Contracts/blob/b8220ee9335167fd3e5d6c5691b20a3a4bb2a3dc/contracts/PrimeFactory.sol#L79-L102) and `getPrimeReplicationByTokenId()` on [L110-L123](https://github.com/Dapp-Wizards/Avastars-Contracts/blob/b8220ee9335167fd3e5d6c5691b20a3a4bb2a3dc/contracts/PrimeFactory.sol#L110-L123) in PrimeFactory, and `getReplicantByTokenId()` on [L64-L85](https://github.com/Dapp-Wizards/Avastars-Contracts/blob/b8220ee9335167fd3e5d6c5691b20a3a4bb2a3dc/contracts/ReplicantFactory.sol#L64-L85) in ReplicantFactory.

##### Description

These four functions are vulnerable to a subtle bug that enables unauthorized data access. While no immediate consequences have been uncovered, this issue comes very close to unlocking potentially severe issues such as unauthorized access to write (or overwrite) to storage.

The issue depends on Primes and Replicants sharing a common Avastar data structure, while also maintaining independent siloed state. If there are more Primes than Replicants in a given Generation, or vice versa, it is possible for a malicious actor to gain unauthorized access to the array of one wave using a token id of the other wave.

For example, consider a scenario in which there are 5000 Primes and 3000 Replicants in `Generation.ONE`. The total Avastar count is 8000. A malicious user owns a Prime with a `tokenId` of #1500 and a `serial` of #1000. This user calls `getReplicantByTokenId(1500)` with their Prime. The only explicit check in easily passes.

```solidity
require(_tokenId avastars.length);
```

The Avastar struct is loaded in memory and its generation and serial are used to access the Replicant struct. Even though this token is aPrime, if its values are in range of any Replicant, regardless of owner, it can gain unauthorized access to this data structure.

Potential damage from accessing a view function in this manner is likely negligible, however, some functions use a similar pattern to make state changes. Consider a scenario with more Replicants in a Generation than Primes. `approveTraitAccess()` does not verify if the `tokenId` is a Prime or Replicant. Owners can store approvals for access to a Replicant's nonexistent `replicated` array. Out-of-bounds array access can have serious consequences, including write access to any storage value anywhere in the contract, even outside of the array.

Fortunately `useTraits()` prevents Replicants from using their traits with

```solidity
require(avastar.wave == Wave.PRIME);
```

While satisfactory at the moment, this single line feels fragile as protection against a potentially serious issue. For example, an uninitialized Avastar defaults to `Wave == 0` (PRIME). If a malicious user is able to bypass this type of check and continue in the function, another user's `Prime.replicated` array could be accessed, altered, and/or overwritten.

`getPrimeReplicationByTokenId()` also allows an imposter Replicant token to load another Prime's `replicated` array in memory. `getPrimeByTokenId()` and `getReplicantByTokenId()` are called externally by the AvastarMetadata contract, but successfully validated by wave. They are not validated if called directly in the AvastarTeleporter contract.
...
- Considering adding an uninitialized value to the Wave enum (similar to Gender) to reduce to risk of erroneous validation of uninitialized tokens as `Wave.PRIME`
...
> 05/02/20
>
> Acknowledged a with request for clarification about the risk level and reproducible threat examples. New unit tests were added to check for failing conditions.

After further analysis I believe that the risk level in this function is low. The trusted set up between the two contracts eliminates some risk of not validating the return values of an external call. I was not able to identify a case in which `mintPrime()` failed without bubbling up to `purchasePrime()`. I was not able to reproduce a case in which `purchasePrime()` failed without reverting the execution of `mintPrime()`.

I was able to reproduce a (highly contrived) edge case in which `purchasePrime()` receives a longer than expected return value and incorrectly parses the result, leading to incorrect data being logged for `tokenId` and `serial`. The likelihood of this case occurring increases if the AvastarTeleporter contract is untrusted or modifies the behaviour of `mintPrimes()`.

Consider the example of a modified `mintPrime()` function that returns 128 bytes of garbage data. `purchasePrime()` accepts this data without generating an error or stopping execution, despite being longer than the expected 64 bytes. It still parses the first and second 32 byte words as `uint256` regardless of the original data type.

```solidity
// EXAMPLE
// this function has the same 4 byte signature as the real `mintPrime()` but returns garbage data for illustrative purposes
...
There have also been changes to the input validations in these functions which alters the set of possible outcomes. `mintPrime()` and `mintReplicant()` can now successfully mint a token with a Gender of `ANY`. A minter level account can do this by accident or maliciously. If such a token is minted, its metadata will display as female based on the ternary operation on L261 and L289 of AvastarMetadata.

```solidity
metadata = strConcat(metadata, (gender == Gender.MALE) ? ' Male ' : ' Female ');
```

`mintReplicant()` also removes a check that the owner is not `address(0)`. However, this is validated again in the ERC-721 `_mint()`. This redundancy allows the function to revert as expected when `address(0)` is passed as the token owner.

##### Location

L217, `purchasePrime()`, AvastarPrimeMinter.

##### Description

User-driven token minting of Primes is initialized in the AvastarPrimeMinter contract. A user deposits enough ether to cover the price of the token they wish to purchase (validated off-chain). A worker address with minter permissions they calls `purchasePrime()` on behalf of the user, passing the token information. The user's balance is debited and the call is forwarded to the AvastarTeleporter contract to mint the token.
...
The best practice of the "Check-Effects-Interaction Pattern" would guide that this call should be explicitly validated. If it fails for any reason, the `purchasePrime()` function should reverse the changes to the user and global ether balances.

As presently written, this function returns two expected `uint256` values from the Teleporter contract. Under certain circumstances, however, it is possible for the external sub-call to fail or return unexpected values without the main transaction reverting. In such a case, the user's ether balance, the ether balance available to the owner to withdraw, and the received serial and tokenId values can all be erroneously changed in the contract's storage.

For example, if the Teleporter returns data of an incorrect length or type, PrimeMinter attempt to convert this into two `uint256` values and still considers the transaction a success.

##### Recommendation

There are several options to validate the return value of this call. The first involves using using the lower-level call version, which exposes access to two return values. A `bool` indicates whether the call succeeded and a `bytes memory` value returns any data from the function being called. The later can be explicitly checked for the expected length of two `uint256` return values.

> 05/02/20
> This section of code replaces an under-explained example. The logic of the function is:

- perform the same validations as the original function
...
- Parse the data with `abi.decode()`
- Emit same event as original function
- Return `traitId` and `serial` as expected. Because they are named return variables they do not need to be explicitly returned in the function body with `return (traitId, serial)`
...
- Note: `enum` types are encoded as the smallest `uint` type that can fit all their possible values (`uint8` in this case)
- The resulting `calldata` constructed by this function and sent to AvastarTeleporter is exactly the same as: `"teleporterContract.mintPrime(_purchaser, _traits, currentGeneration, currentSeries, _gender, _ranking)"`
