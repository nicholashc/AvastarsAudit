...
Creating and validating the trait hash for each Avastar happens entirely off-chain. There are a siginificant number of possible hashes which do not map to valid traits. The functions `assembleTraitMetadata()` and `renderAvastar()` have no mechanism for parsing invalid genes other than omitting them. The application relies on an external validation systems and is not enforced at the smart contract level.
...
Acknowledged. External validation of the arguements passed to `createTrait()` is part of the application's trust model.
...
By reorganizing the lexical order of declaration, it's possible to tightly pack the variables into one fewer storage slot. This will reduce the gas cost for each new trait created by 20k using one fewer `SSTORE` opcode.
...
In various parts of the code base variables are declared with the `uint` alias. They should be replaced with `uint256` to favour of explicitness and to avoid unintentional implicit type conversion.
...
The function `strConcat()` on [L42-L46](https://github.com/Dapp-Wizards/Avastars-Contracts/blob/b8220ee9335167fd3e5d6c5691b20a3a4bb2a3dc/contracts/AvastarBase.sol#L42-L46) in AvastarBase uses `abi.encodePacked()` with two with dynamic length types. The current implementation is benign. However, if the same function is reused in the future with for some other purpose that incldues a hash, its output can produce collisions.
...
- Off-chain hash generation, including random number genereration
- Minting management system
- Key managemenet system for permissioned accounts
- Pricing system
- AvastarsReplicantMinter smart contract

### A02: Test Coverage

This test coverage represents the unit test coverage as of the [b8220ee](https://github.com/Dapp-Wizards/Avastars-Contracts/tree/b8220ee9335167fd3e5d6c5691b20a3a4bb2a3dc) commit. Test coverage has increased as developement continued. The test coverage report was generated with [solidity-coverage](https://github.com/sc-forks/solidity-coverage).
...
Moderate severity issues or issues that may only be expoitable as edgecases. These issues should be addressed. Examples include:
...
Low severity security issues with minor impact. This section may also include substaintial optimization or other recommendations. In some cases, these issues may represent the auditors subjective attitude about best practices, design decisions, and/or contract architecture.
...
- Deviations from smart contract developement or Solidity best practices
...
| :------- | :------------------------------------------ | :------ | :------------------------------------------------------------------------------------------------------------ |
| 05/23/20 | Avastars Smart Contract Audit Public Report | 0.3.0   | N/A                                                                                                           |
